...
- 6fda287: Extract a shared `AbstractPostgresJournalPersistenceAdapter` (positional `$N` SQL over role-based query hooks) and a shared `decodeBytea` helper in xjog-util; journal-pg/journal-pglite keep only their driver/connection layer.
